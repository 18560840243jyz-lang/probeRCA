...
Burst健康校准必须按`(entity_id, channel_id)`分别冻结，禁止把不同服务、主机或
TCP边混为一个参考分布。计数通道有真实暴露量时使用`count/exposure`；暴露量为0
时保留该1秒窗的真实count，禁止除以epsilon制造超大事件率。稀有事件阈值取配置
分辨率下限与Healthy高分位的较大值，且在查看故障标签前冻结。
