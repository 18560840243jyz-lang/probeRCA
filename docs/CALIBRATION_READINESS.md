...
- Confirmed Hard：同一实体的症状分数连续 3 个 1 秒窗口不低于 5；只有它进入
  Hard、触发正式RCA，并使Healthy Validation失败。单个正式服务、主机或TCP边可以独立确认，
  不要求多实体佐证。

除服务请求和TCP症状分数外，正式服务/主机还使用健康校准的稳定资源变化分数。
变化量相对严格在前的30窗滚动中位数计算；每个指标名的Hard变化阈值只从
CALIBRATING段学习并冻结。两窗预过滤只用于排除3窗瞬时资源尖峰，之后仍使用上述
公共Soft/Hard状态机。该通道不能读取注入标签，也不能按具体实体硬编码。
