...
class ResourceAlertChannel:
    """Detect sustained changes in stable resource occupancy/pressure metrics.
    ...
    common Soft/Hard state machine and its thresholds remain unchanged.
    """
    ...
    def __init__(self, config: FinalControlConfig):
        ...
        self._calibration_worst: dict[str, float] = {
            name: 0.0 for name in config.resource_alert_metric_names
        }
    ...
    def reset(self) -> None:
        ...
        self._calibration_worst = {
            name: 0.0 for name in self.config.resource_alert_metric_names
        }
    ...
    def freeze(self) -> None:
        ...
        self._thresholds = {
            name: max(
                ...
                self._calibration_worst.get(name, 0.0)
                + self.config.resource_alert_calibration_margin,
            )
            for name in self.config.resource_alert_metric_names
        }
        self._prefilter_counts.clear()
        ...
        for node_id, observation in sorted(observations.items()):
            ...
            if learn:
                ...
                if len(run) == self.config.hard_consecutive_windows:
                    self._calibration_worst[name] = max(
                        self._calibration_worst[name], min(run),
                    )
            if self._thresholds is None:
                continue
            normalized = (
                self.config.hard_threshold * delta / self._thresholds[name]
            )
            count = (
            ...
            details[node_id] = {
                "metric_name": name,
                "rolling_delta": delta,
                "learned_hard_delta": self._thresholds[name],
