...
表示逻辑TCP尝试数：已完成应用事务数加上在建立应用事务前终止的连接尝试数；
[
x_{g,\mathrm{latency}}(t)
]
表示已完成应用事务的边延迟P95；预连接失败没有延迟观测，禁止将超时上限或0写入直方图；
[
x_{g,\mathrm{failure}}(t)
]
表示已完成事务错误/超时与预连接失败之和除以逻辑TCP尝试数。

正式Normal实现必须同时使用两个彼此独立、无标签的累计来源：Beyla提供已完成应用
事务及其延迟直方图；常驻Normal eBPF按源cgroup和目标地址/端口累计在
`SYN_SENT`或`SYN_RECV`阶段关闭的预连接失败。Normal不得从Burst事件反推这些
计数。为了校验直方图，数据面可以保留“已完成事务数”这一内部原始组件，但它不得成为
第4个正式边指标、根因坐标、告警坐标或FISTA变量。无法映射到冻结身份的原始计数不得
猜测归属。
...
服务请求延迟/失败和TCP边延迟/失败继续使用原有症状分数。为避免正式资源根因
已经强异常但没有立即传播成请求症状时永远无法启动RCA，控制面还维护一个统一的
稳定资源变化通道。该通道只使用配置声明的连续占用/压力指标，不读取故障标签：

- 使用当前封存观测段自身的前30个有效窗口建立短期滚动中位数；
- 资源变化量为当前异常分数减去严格在前的滚动中位数，结果截断到非负；
- 每个指标名的Hard变化阈值只从CALIBRATING健康段学习并冻结；
- 两个连续资源变化窗口仅作为瞬时尖峰预过滤，随后仍进入统一的Soft/Hard按实体
  连续计数；因此3窗短暂资源尖峰不能单独形成Confirmed Hard；
- 当前正式稳定资源指标集合从冻结配置读取，不允许按服务、主机或故障类型特判；
- 稀疏事件指标和请求/TCP症状仍走各自已有通道，不得为了触发率补零或复用旧值。
...
- 任何健康负载调度语义修正必须升版load profile、记录原因，并重新运行独立Healthy Pilot；
  不得以此修改Soft/Hard、family floor或其他控制面门禁。
...
- 进入准确率分母前，必须在封存后单独验证：注入作用于真实应用/资源/通信路径；声明的正式根因指标或同一正式机制确实被观测；异常期通过请求/TCP症状通道或健康校准的稳定资源变化通道形成正式Confirmed Hard；拓扑、runtime identity与Pod restart保持稳定；没有人为叠加第二类故障。
